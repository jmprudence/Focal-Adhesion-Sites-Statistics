...
#23 May    2022 - Added underestimate simulation 
...
#11. FA sites are eliminated based on user input for one group 
...
for(r in 1:reps){
  ...
  path <- #Enter path to folder containing small group files 
...
for (i in 1:length(file_list)){
  ...
  #Choosing Ratio  
  falist[[i]] <- ppp(fx[[1]], fy[[1]], window = w, marks = area) 
  
  group <- na.omit(eval(parse(text = file_list[i]))[1,9])
  
  #If the group is not population, thin sites based on user input 
  if(group != "population"){
    falist[[i]] <- ppp(fx[[1]], fy[[1]], marks = area, window=w)
