...
#21 June   2022 - Added manipulation of mark data randomly between two points
...
LBound <- readline(prompt = "Enter Lower Boundary of Change:")
LBound <- as.numeric(LBound)
UBound <- readline(prompt = "Enter Upper Boundary of Change:")
...
for(r in 1:reps){
  ...
  path <- #Enter path to the large group files 
  ...
  path <- #Enter path to the small group files
...
for (i in 1:length(file_list)){
  ...
  #If the group is not the population, find the length of the FA list 
  #Create a list of random number between the user inputted bounds 
  #multiply area list by the list of random numbers for perturbed area
  if(group != "population"){
    LengthFA <- length(fx[[1]])
    change <- runif(LengthFA, LBound, UBound)
    area <- area * change
