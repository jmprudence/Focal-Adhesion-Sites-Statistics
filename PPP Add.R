...
# This iteration is designed to simulate the overestimate of FA sites by a 
...
#04 April  2022 - Added random sampling
#30 April  2022 - Added overestimate of FA sites

### Basic Overview-Steps:
 #1.  Randomly Samples from two populations and reads in FA and membrane (x,y) 
 ###  data and group (actual or ml) from a .csv file
 ...
 #11. Overestimate of FA sites is conducted through overlaying the original and a random ppp
 #12. Next, a hyperframe is built
 #13. Next, Hahn's studentized permutation is used to compare two populations
...
results_folder <- #Enter Results Folder
...
PerAdd <- readline(prompt ="Enter Percentage of Over Estimate as Decimal (i.e. 1% is 0.01):")
PerAdd <- as.numeric(PerAdd)
pppAdd <- replicate(reps, NULL)
...
for(r in 1:reps){
  ...
  unlink#( Enter Random Sampling Folder , recursive=TRUE)
  ...
  path <- #Enter path to large group folder
  ...
  path <- #Enter path to small group folder
  ...
  for (i in 1:length(file_list)){
    ...
    group <- na.omit(eval(parse(text = file_list[i]))[1,5])
  
    # If the group is not the population, random ppp is created with percentage 
    # of original sites based on user input. PPP is combined with original
    if(group != "population"){
      Add <- npoints(falist[[i]])*PerAdd
      Add <- round(Add, 0)
      pppAdd[[i]] <- runifpoint(n=Add, win=w)
      falist[[i]] <- unique(superimpose(falist[[i]], pppAdd[[i]] , W = w))
    }else{    }
  ...
  #will title each PPP with the group to indicate the source of the cells. 
  #"ml"= machine learning prediction
  #"actual"=cells used to testing the machine learning algorithm. Shares membranes with "ml" group
  #"population"=cells used to train the algorithm
  fac <- focal_adhesion
  fac$grp <- abbreviate(focal_adhesion$group, minlength=20)
  plot(fac, quote(plot(FA_centroids, pch=16, main=grp, cex = 0.5, pch = "+")), main="Focal Adhesion Centroids", arrange=FALSE)
