...
# This iteration is designed to simulate the underestimate of FA sites by a 
...
### Versions:
  ...
  #23 April  2022 - Added Elimination of Points to simulate underestimating the number of FA Sites
...
### Basic Overview-Steps:
  ...
  #11. Underestimate of FA sites conducted through rthin function
...
results_folder <- #Enter folder for the results
...
for(r in 1:reps){
  ...
  unlink#(Enter path for the random sampling, recursive=TRUE)
  ...
  pwd=setwd # Endter path to working directory
  ...
  #results       = summary statistics for studentized permutation test
  ...
  for (i in 1:length(file_list)){
    ...
    #If the group is not the population, thin PPP based on user input 
    if(group !="population"){
      falist[[i]] <- ppp(fx[[1]], fy[[1]], window=w)
