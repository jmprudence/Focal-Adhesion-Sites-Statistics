...
# This iteration randomly samples files from a pool of files 
...
results_folder <- ### Enter Location of Results Folder ###
...
for(r in 1:reps){
  ...
  unlink#(### Enter Path to Random Sample Folder, recursive=TRUE)
...
path <- #### Enter Path to Random Sample Folder
...
path <- #### Enter Path to Random Sampel File ###
...
pwd=setwd#(### Enter Working Directory ###)
...
#results       = list of summary statistics for studentized permutation test
...
results[[r]] <- studpermu.test(focal_adhesion, FA_centroids ~ group, nperm=99, minpoints = 10)
