...
keep_site <- readline(prompt ="Enter Probability Site is Retained in Small Group:")
keep_site <- as.numeric(keep_site)
...
for(r in 1:reps){
  ...
  path <- "C:/Users/jayre/Documents/Focal Adhesion Statistics/GT Marks Pop/"
  ...
  path <- "C:/Users/jayre/Documents/Focal Adhesion Statistics/GT Marks Copy/"
...
for (i in 1:length(file_list)){
  ...
  #Choosing Ratio  
  falist[[i]] <- ppp(fx[[1]], fy[[1]], window = w, marks = ratio) #JP
  
  group <- na.omit(eval(parse(text = file_list[i]))[1,9])
  
  if(group != "population"){
    falist[[i]] <- ppp(fx[[1]], fy[[1]], marks = ratio, window=w)
    falist[[i]] <- rthin(falist[[i]], keep_site, nsim=1, drop=TRUE)
  }else{ }
...
# plot(fac, quote(plot(FA_Location, main=group, markscale=0.01)), main="Focal Adhesion Sites with Marks", arrange=FALSE) #JP
