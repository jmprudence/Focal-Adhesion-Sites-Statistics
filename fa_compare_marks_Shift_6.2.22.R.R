...
for(r in 1:reps){
  ...
  path <- "~/Focal Adhesion Statistics/Population Marks/"
  ...
  path <- "~/Focal Adhesion Statistics/Actual Marks"
...
for (i in 1:length(file_list)){
  ...
  #build ppp 
  falist[[i]] <- ppp(fx[[1]], fy[[1]], window = w, marks = area[[1]]) #JPS
...
# plot(fac, quote(plot(FA_Location, main=group, markscale=0.01)), main="Focal Adhesion Sites with Marks", arrange=FALSE)
