...
#statistically different using a studentized permutation test of Hahn (2012). One population
#is the actual(true) population, while the second is the machine-learning-predicted 
#population.
}
### Key Reference(s):
...
#November 2021  - Add Marks Processing JP
}
### Basic Overview-Steps:
{#1.  Reads in FA and membrane (x,y) data and group (actual or ml) from a .csv file
#2.  The .csv header format is the following: xfa yfa xmem ymem mark group JP
...
results_folder <- "C:/Users/jayre/Documents/Focal Adhesion Statistics/RandomSample/"

### Inputs
reps <- readline(prompt="Enter Repetitions: ")
reps <- as.numeric(reps)
results <- replicate(reps, NULL)
percent_number_big <- readline(prompt="Enter Number of Large Group Files:")
percent_number_small <- readline(prompt="Enter Number of Small Group Files:")
PerChng <- readline(prompt ="Enter Percentage of Mark Size (i.e. 1% larger is 1.01):")
PerChng <- as.numeric(PerChng)
...
for(r in 1:reps){
  ...
  unlink("C:/Users/jayre/Documents/Focal Adhesion Statistics/RandomSample", recursive=TRUE)
  ...
  path <- "C:/Users/jayre/Documents/Focal Adhesion Statistics/Population Marks/"
  ...
  path <- "C:/Users/jayre/Documents/Focal Adhesion Statistics/Actual Marks/"
  percent_number_small <- as.numeric(percent_number_small)
  random_files_small(path, percent_number_small, pattern = "*.csv")

    ### End Function Section

# set current working directory to read/write data, use / JP
pwd=setwd("C:/Users/jayre/Documents/Focal Adhesion Statistics")

# set path to directory that with multiple .csv files of input data, use /, add / after last word JP 
folder <- "C:/Users/jayre/Documents/Focal Adhesion Statistics/RandomSample/"
...
for (i in 1:length(file_list)){
  ...
  width  <- as.list(eval(parse(text= file_list[i]))[3]) #Find the mark data #JP
  height <- as.list(eval(parse(text= file_list[i]))[4]) #Find the mark data #JP
  area   <- as.list(eval(parse(text= file_list[i]))[5]) #Find the mark data #JP
  ratio  <- as.list(eval(parse(text= file_list[i]))[6]) #Find the mark data #JP
  
  width  <- na.omit(eval(parse(text = file_list[i]))[3]) #JP
  height <- na.omit(eval(parse(text = file_list[i]))[4]) #JP
  area   <- na.omit(eval(parse(text = file_list[i]))[5]) #JP
  ratio  <- na.omit(eval(parse(text = file_list[i]))[6]) #JP

  #Group that the cells are from
  
  group <- na.omit(eval(parse(text = file_list[i]))[1,9])
  
  # #Choosing Ratio
  # falist[[i]] <- ppp(fx[[1]], fy[[1]], window = w, marks = ratio) #JPS
  # 
  # if(group != "population"){
  #   ratio <- ratio * PerChng
  #   falist[[i]] <- ppp(fx[[1]], fy[[1]], window = w, marks = ratio)
  # }else{    }
  
  #Choose Area
  #Normalize area - Did not use this, but was considering
  # area <- lapply(area, function(x){(x)/(xmax-xmin)}) #normalize area
  # area <- lapply(area, function(x){(x)/(ymax-ymin)})
  ...
  if(group != "population"){
    area <- lapply(area, "*", PerChng)
    falist[[i]] <- ppp(fx[[1]], fy[[1]], window = w, marks = area[[1]])
  }else{ }

  #build group list of which group each data set belongs to
  if(i==1){glist <- na.omit(eval(parse(text = file_list[1]))[1,9]) #JP
  }else{
    glist <- c(glist, na.omit(eval(parse(text = file_list[i]))[1,9])) #JP
  }
}
...
# plot(fac, quote(plot(FA_Location, main=group, markscale=0.001)), main="Focal Adhesion Sites with Marks", arrange=FALSE) #JP

#run studentized permutation test from Hahn (2012) for differences in spatial point patterns
#and plot results
results[[r]] <- studpermu.test(focal_adhesion, summaryfunction = Kmark, minpoints = 1, nperm=99)
